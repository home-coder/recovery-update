...
static char ota_md5_result[32];
static unsigned int package_recovery = 0;
static unsigned int ota_recovery = 0;
...
get_args(int *argc, char ***argv) {
    ...
    if (!strncmp(boot.sign, "recovery", strlen("recovery"))) {
        memcpy(ota_md5_result, boot.md5_result, sizeof(ota_md5_result));
        memset(boot.sign, 0, sizeof(boot.sign));
        memset(boot.md5_result, 0, sizeof(boot.md5_result));
        ota_recovery = 1;
    }
...
main(int argc, char **argv) {
    ...
    if(!strcmp("usb-recovery",BOOT_COMMAND)){
        ...
        if(!search_file_in_usb("update.zip", absolutePath))
            ...
            recovery_status |= (1 << INSTALL_ERROR);
    }

    if(!strcmp("wipe-data",BOOT_COMMAND))
        wipe_data = 1;
    ...
    if (update_package) {
        ...
        if (strncmp(update_package, "CACHE:", 6) == 0) {
            ...
            update_package = modified_path;
            recovery_status &= ~(1 << INSTALL_ERROR);
        } else if (strncmp(update_package, "usb:update", 10) == 0) {
    ...
    if (ota_recovery && (update_package != NULL)) {
        int i;
        char file_md5[MD5_DIGEST_LENGTH * 2 + 1];

        ensure_path_mounted(update_package);
        if (do_md5(update_package, file_md5) == 0) {
            for (i = 0; i < MD5_DIGEST_LENGTH * 2; i++) {
                file_md5[i] ^= md5_XOR[i];
            }
            if (!memcmp(ota_md5_result, file_md5, sizeof(ota_md5_result)))
                package_recovery = 1;
        }
    }
    ...
    printf("\n");
      ...
      if (package_recovery && (update_package != NULL)) {
        ret = install_package(update_package, &wipe_cache, TEMPORARY_INSTALL_FILE);
        if  (secureBootFlag==0 && ret == INSTALL_SUCCESS)
            secureBootFlag=1;
        ...
        recovery_status |= (1 << ret);
        //sync fs
        sync();
        sleep(5);
    }

    if (recovery_flag && wipe_data) {
        ret = WIPE_SUCCESS;
        if (device->WipeData()) ret = WIPE_ERROR;
        if (erase_volume("/data")) ret = WIPE_ERROR;
        erase_volume("/Reserve0");
        copy_databk_to_data();
        if (ret != WIPE_SUCCESS) ui->Print("Data wipe failed.\n");
        recovery_status |= (1 << ret);
    }

    if (recovery_flag && wipe_cache) {
        ret = WIPE_SUCCESS;
        if (erase_volume("/cache")) ret = WIPE_ERROR;
        if (ret != WIPE_SUCCESS) ui->Print("Cache wipe failed.\n");
        recovery_status |= (1 << ret);
    }

    if (recovery_flag && wipe_full) {
        ret = WIPE_SUCCESS;
        if (erase_volume("/data")) ret = WIPE_ERROR;
        if (ret != WIPE_SUCCESS) ui->Print("Data wipe failed.\n");
        if(volume_for_path("/sdcard")!=NULL){
            if (format_volume("/sdcard")) ret = WIPE_ERROR;
            if (ret != WIPE_SUCCESS) ui->Print("sdcard wipe failed.\n");
        }
        if (erase_volume("/cache")) ret = WIPE_ERROR;
        if (ret != WIPE_SUCCESS) ui->Print("Cache wipe failed.\n");
        if (ret==WIPE_SUCCESS&&set_fatcory_flag()==0)
            factory_wipe_ok = 1;
        if (!factory_wipe_ok)
        {
    ...
    if (!just_exit)
        recovery_status |= (1 << INSTALL_NONE);  // No command specified

    if ((recovery_status & (1 << INSTALL_ERROR)) ||
        (recovery_status & (1 << INSTALL_CORRUPT)) ||
        (recovery_status & (1 << WIPE_ERROR))) {
        ui->SetBackground(RecoveryUI::ERROR);
        ui->set_led_flicker(ui->LED_ERROR);
        status = INSTALL_ERROR;
    } else if (recovery_status & (1 << INSTALL_SUCCESS)) {
        ui->SetBackground(RecoveryUI::INSTALLING_SUCCESS);
        status = INSTALL_SUCCESS;
    } else if (recovery_status & (1 << WIPE_SUCCESS)) {
