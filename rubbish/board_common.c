...
#define IR_SPRITE_RECOVERY_KEY_VALUE		(0X83)
#define IR_BOOT_RECOVERY_VALUE				(0x88)
#define IR_BOOT_RECOVERY_WIPE_DATE_VALUE    (0x89)
/*
************************************************************************************************************
*
*                                             function
...
static int detect_sprite_boot_action(void)
{
	int key_value;
	...
	else if ((key_value == SPRITE_RECOVERY_KEY_VALUE) || (key_value == IR_SPRITE_RECOVERY_KEY_VALUE))
	{
	...
	else if (key_value == IR_BOOT_RECOVERY_WIPE_DATE_VALUE)
	{
		printf("ir recovery set to ir boot recovery wipe data\n");
		return ANDROID_RECOVERY_MODE;
	}
...
int check_android_misc(void)
{
	...
	mode = detect_sprite_boot_action();
#else
	...
	else if(mode == ANDROID_SPRITE_MODE)
	{
...
************************************************************************************************************
*/
int write_usb_recovery_to_misc(char *command)
{
	u32   misc_offset = 0;
	char  misc_args[2048];
	static struct bootloader_message *misc_message;
	...
	strcpy(misc_message->command, command);
...
int respond_ir_boot_recovery_action(void)
{
	...
	if (ret == 1)
	{
	    ret = script_parser_fetch("ir_boot_recovery", "ir_work_mode", (int *)&work_mode, sizeof(int) / 4);
	    if(ret || (work_mode == 0))
        {
		    printf("to ir boot recovery\n");
            gd->key_pressd_value = IR_BOOT_RECOVERY_VALUE;
        }
        else if(work_mode == 1)
        {
            printf("to ir one-key recovery mode\n");
            gd->key_pressd_value = IR_SPRITE_RECOVERY_KEY_VALUE;
	...
	{
		printf("ir boot recovery wipe date\n");
		gd->key_pressd_value = IR_BOOT_RECOVERY_WIPE_DATE_VALUE;
		return 0;
	}
...
void respond_physical_key_action(void)
{
	...
	{
		printf("[box recovery] set to one key usb recovery\n");
		write_usb_recovery_to_misc("usb-recovery");
	}
	else if (key_value == IR_BOOT_RECOVERY_VALUE)
	{
		printf("[ir recovery] set to ir boot recovery\n");
	...
	{
		printf("ir recovery set to ir boot recovery wipe data\n");
		write_usb_recovery_to_misc("wipe-data");
	}
