...
int
do_md5(const char *path, char *outMD5)
{
	unsigned int i;
	int fd;
	MD5_CTX md5_ctx;
...
static int
parseUpdateExCmd(const char *opt, char *relative_path, char *md5, char *syslink)
{
	char tmp[2000];
	char *pstart, *pend;
	strcpy(tmp, opt);
...
static int
getNodeBySysLnk(char *syslink, char *outNode)
{
	dirent *dirp;
	DIR *dp;
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		if (S_ISLNK(file_stat.st_mode)) {
			...
			fprintf(stdout, "getNodeBySysLnk:%s %s\n", file,
				tmplink);
			//比较其设备路径和设备号
			if (strncmp(syslink, tmplink, SYSLNK_MAX_TEST_DEPTH) ==
			    0
			    && strcmp(syslink + strlen(syslink) - 1,
				      tmplink + strlen(tmplink) - 1) == 0) {
				count =
				    sscanf(dirp->d_name, "%lld:%lld", &major,
					   &minor);
				break;
			}
		}
...
static int
testFile(char *relative_path, char *md5)
{
...
int
updateFromMutliDevice(const char *opt, char *outpath)
{
	...
	while (times--) {
		if (getNodeBySysLnk(syslink, node) == 0) {
			fprintf(stderr,
				"Get node by syslink %s failed, try again!Left %d\n",
				syslink, times);
			sleep(1);
			continue;
		}
	...
	if (symlink(tmp, CACHE_LINK_PATH)) {
		fprintf(stderr, "Link %s to %s error, %s", tmp, CACHE_LINK_PATH,
			strerror(errno));
		return 0;
	}
